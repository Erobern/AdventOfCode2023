    ...
    public static String Puzzle2(String input) {
        ...
        while (!nextNodes.isEmpty()) {
            ...
            nodesToEvaluate.forEach(node -> {
                ...
                // try to populate the next potential nodes
                try {
                    Coordinate noStep = node.startCoordinate().step(node.startDirection(), 0);
                    newHeatLost += grid[noStep.line()][noStep.column()];
                ...
                for (int i = 3; i < 10; i++) {
                    try {
                        Coordinate threeStep = node.startCoordinate().step(node.startDirection(), i);
                        newHeatLost += grid[threeStep.line()][threeStep.column()];

                        if (threeStep.line().equals(finalCoordinate.line()) && threeStep.column().equals(finalCoordinate.column())) {
                            if (newHeatLost < bestResult.get()) {
                                bestResult.set(newHeatLost);
                                bestResultPath.set(node.history());
                            }
                        ...
                        nextNodes.add(new Node(threeStep.step(node.startDirection().clockwise(), 1), node.startDirection().clockwise(), newHeatLost, node.history + ">" + node.getHashKey()));
                        nextNodes.add(new Node(threeStep.step(node.startDirection().counterClockwise(), 1), node.startDirection().counterClockwise(), newHeatLost, node.history + ">" + node.getHashKey()));
                    } catch (Exception ignored) { return; }
                }
